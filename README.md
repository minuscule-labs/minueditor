...
Rendered diagrams use explicit inline pan, zoom, and reset controls so trackpad gestures continue scrolling the document. **Expand** opens a large modal where drag/touch panning and wheel or pinch zooming are enabled. Keyboard users can pan with the arrow keys, zoom with `+`/`-`, reset with `0`, and close the modal with Escape. This behavior is available in both the editor and static renderer, is enabled by default, and can be disabled with `mermaid={{ interactive: false }}`.
